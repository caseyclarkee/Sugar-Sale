// sugar-cursor.js
(() => {
  const canvas = document.getElementById('sugar-stage');
  const ctx = canvas.getContext('2d');
  const cursor = document.getElementById('sugar-cursor');
  ...
  function grain(x, y, burst = false) {
    ...
    return {
      x, y,
      vx: Math.cos(a) * sp,
      vy: burst ? Math.sin(a) * sp : (1.0 + Math.random() * 0.6),
  ...
  function drawTrianglePile(x, y, mass) {
    ...
    for (let i = 0; i < dots; i++) {
      ...
      ctx.beginPath();
      ctx.arc(px + (Math.random() * 0.6 - 0.3), py + (Math.random() * 0.6 - 0.3), r, 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.globalAlpha = 1;
  }
